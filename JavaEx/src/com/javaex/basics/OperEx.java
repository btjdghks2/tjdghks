...
public class OperEx {
	...
	// 비교연산, 논리 연산
	// 결과로 boolean  반환, 논리 값으로 프로그램의 흐름 제어 -> 중요
	private static void logicOper() {
			// qlry dustkswk >,>=,<,<=,==(같다), !=(같지 않다)
		int n1 = 7;
		int n2 = 3;
		...
		System.out.println("n3가 0이하이거나 n3가 10 이상의 영역?" + r1orr2);
		
		// not 논리부정 -> trun <-> false 반전
	...
	// 정수 / 정수 -> 정수
		//실제 나눗셈의 결과는 ?
		System.out.println((float)a /(float)b); // int -> float 로 변함
		System.out.println((float)a/ b); // b는 float로 자동변환
		...
		a = 7;
		System.out.println("a:" + a);
		System.out.println("후위 증감연사 :" + (a++));
		System.out.println("최종 a : " + a);
		
		// 나눗샘 보충 
		a = 7;
	//	System.out.println( a / 0); -> 정수를 0으로 나누면 ArithmeticExcepyion
		...
		System.out.println( 0.0 / 0.0 + 10); //그래도 NaN
